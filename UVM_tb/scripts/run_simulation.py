...
def run_rtl_simulation(mem_file, rtl_log_file):
    """Run the RTL simulation with Vivado Xsim"""
    print("--- Running RTL Simulation ---")

    # Base xsim command
    # Assuming 'smoke_top' snapshot has been built by 'make elaborate'
    xsim_cmd = [
        "xsim",
        "smoke_top",
        "-R",
        f"-testplusarg", f"ram_init_file={mem_file}",
        f"-testplusarg", f"trace_log={rtl_log_file}",
    ]
    ...
    try:
        ...
        print(
            "Error: 'xsim' not found. Is Vivado installed and in PATH?",
            file=sys.stderr,
        )
        sys.exit(1)
