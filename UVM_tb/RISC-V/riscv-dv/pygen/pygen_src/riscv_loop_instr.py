...
class riscv_loop_instr(riscv_rand_instr_stream):
    ...
    def post_randomize(self):
        ...
        for i in range(self.num_of_nested_loop):
            ...
            try:
                self.loop_init_instr[2 * i] = riscv_instr.get_rand_instr(
                    include_instr=[riscv_instr_name_t.ADDI])
                self.loop_init_instr[2 * i].randomize()
                self.loop_init_instr[2 * i].rd = int(self.loop_cnt_reg[i])
                self.loop_init_instr[2 * i].rs1 = int(riscv_reg_t.ZERO)
                self.loop_init_instr[2 * i].imm = int(self.loop_init_val[i])
            ...
            try:
                self.loop_init_instr[2 * i + 1] = riscv_instr.get_rand_instr(
                    include_instr=[riscv_instr_name_t.ADDI])
                self.loop_init_instr[2 * i + 1].randomize()
                self.loop_init_instr[2 * i + 1].rd = int(self.loop_limit_reg[i])
                self.loop_init_instr[2 * i + 1].rs1 = int(riscv_reg_t.ZERO)
                self.loop_init_instr[2 * i + 1].imm = int(self.loop_limit_val[i])
            ...
            self.loop_update_instr[i] = riscv_instr.get_rand_instr(
                include_instr = [riscv_instr_name_t.ADDI])
            try:
                self.loop_update_instr[i].randomize()
                # Assign loop counter registers and step value after randomization.
                self.loop_update_instr[i].rd = int(self.loop_cnt_reg[i])
                self.loop_update_instr[i].rs1 = int(self.loop_cnt_reg[i])
                self.loop_update_instr[i].imm = int(self.loop_step_val[i])
                ...
                logging.critical("loop_cnt_reg[%d]=%s, loop_step_val[%d]=%s, instr=%s", i,
                                 str(self.loop_cnt_reg[i]), i,
                                 str(self.loop_step_val[i]),
                                 str(self.loop_update_instr[i].instr_name.name))
